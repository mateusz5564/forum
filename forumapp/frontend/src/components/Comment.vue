      ...
      <v-flex class="formText">
        <v-layout justify-space-between row>
          <v-flex shrink>
            ...
            >{{ commentData[0].user.username }}</span>
            <span class="caption ml-1">{{ calculateDate(commentData[0].created_at) }}</span>
            <span class="body-1">
              <a
                v-if="accessToken"
                class="amber--text text--darken-3 font-weight-light"
                @click="expand = !expand"
              > odpowiedz</a>
            </span>
          </v-flex>
          ...
          <v-flex shrink>
            ...
            >{{ mark(commentData[0].number_of_comment_likes, commentData[0].number_of_comment_dislikes) }}</span>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      axios.get(`${API}comments/?id=${this.comment.id}`).then(response => {
    ...
    likeIt() {
      axios
        .get(
          `${API}comments_rating/create/?user=${this.userId}&comment=${
            this.comment.id
          }`
        )
        .then(response => {
